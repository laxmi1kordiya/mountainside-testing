...
function () {
  ...
  ProductDetails_createClass(ProductDetails, [{
    ...
    value: function _switchVariant(event, data) {
      ...
      // Dispatch custom event for other components (e.g., Notify Me button)
    ...
    value: function _updateButton(variant) {
      ...
      // Update data-variant-id attribute when variant changes
      this.$productAtcButton.attr('data-variant-id', variant.id);

      // Update data-inventory-quantity attribute if inventory tracking is enabled
      if (variant.inventory_quantity > 0) {
        this.$productAtcButton.attr('data-inventory-quantity', variant.inventory_quantity);
      } else {
        this.$productAtcButton.removeAttr('data-inventory-quantity');
      }
    }
  }, {
...
//# sourceMappingURL=DynamicFeaturedCollection-StaticCollection-StaticProduct-StaticProductRecommendations-StaticSearch.bundle.js.map?1565720446002