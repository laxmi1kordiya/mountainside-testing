...
function () {
  function StaticCart(section) {
    ...
    // Request queue management for preventing race conditions
  ...
  _createClass(StaticCart, [{
    ...
    value: function _bindEvents() {
      ...
      // Increased debounce to 500ms for better handling of rapid input
      ...
      this.$el.on('change.cart-page', '[data-quantity-select]', function (event) {
        _this2._updatePriceOnQuantityChange(event);
      });
    ...
    key: "_updatePriceOnQuantityChange",
    value: function _updatePriceOnQuantityChange(event) {
      var $target = jquery__WEBPACK_IMPORTED_MODULE_0___default()(event.currentTarget);
      var cartItemRow = $target.closest('[data-cartitem]')[0];
      var selectedQuantity = parseInt($target.val(), 10);
      var unitPrice = parseInt(cartItemRow.getAttribute('data-cartitem-unit-price'), 10);

      if (!isNaN(unitPrice) && !isNaN(selectedQuantity) && selectedQuantity > 0) {
        var newLineTotal = unitPrice * selectedQuantity;
        var priceElement = cartItemRow.querySelector('[data-unit-price-display]');
        var lineTotalElement = cartItemRow.querySelector('[data-cartitem-total]');

        if (priceElement) {
          priceElement.textContent = Shopify.formatMoney(newLineTotal, this.settings.money_format);
        }
        if (lineTotalElement) {
          lineTotalElement.textContent = Shopify.formatMoney(newLineTotal, this.settings.money_format);
        }
      }
    }
    /**
    ...
    value: function _updateCart(key, quantity) {
      ...
      this._pendingUpdate = { key: key, quantity: quantity };

      // Debounce the actual API call by 300ms to catch rapid inputs
      this._updateDebounceTimer = setTimeout(function() {
        _this3._executeUpdate();
      }, 300);
    }
  }, {
    key: "_executeUpdate",
    value: function _executeUpdate() {
      var _this3 = this;

      // If already updating, the pending update will be processed after current completes
      if (this._isUpdating) {
        return;
      }
      ...
      window.Shopify.changeItem(update.key, update.quantity, function (response) {
        _this3._isUpdating = false;

        // Process any pending updates that came in during this request
        if (_this3._pendingUpdate) {
          _this3._executeUpdate();
        } else {
          _this3._didUpdate(update.key, update.quantity, response);
        }
      });
    }
  }, {
    ...
    value: function _didUpdate(id, quantity, response) {
      ...
      // Dispatch custom event with accurate server total for shipping bar
      var cartUpdatedEvent = new CustomEvent('cart:updated', {
        detail: { total_price: response.total_price, items: response.items }
      });
      document.dispatchEvent(cartUpdatedEvent);

      // Also update shipping bar directly if available
      if (window.updateShippingBar) {
        window.updateShippingBar(response.total_price);
      }
      ...
      shopify_asyncview__WEBPACK_IMPORTED_MODULE_4__[/* default */ "a"].load('/cart', '_ajax').done(function (_ref) {
        ...
        // Use event delegation on static parent instead of re-binding
        // Events already bound on $el will work for dynamically added elements
        // Only rebind if the parent element was replaced
    ...
    value: function _bindShippingCalcEvents() {
      ...
      this.$el.on('click.cart-page', '[data-cartshipping-toggle]', function (event) {
        var clickedButton = jquery__WEBPACK_IMPORTED_MODULE_0___default()(event.currentTarget);
        var currentText = clickedButton.text();
        var toggleText = clickedButton.attr('data-cartshipping-toggle');

        clickedButton.text(toggleText);
        clickedButton.attr('data-cartshipping-toggle', currentText);

        _this6.$shipping.toggleClass('open');
      });
...
//# sourceMappingURL=StaticCart.bundle.js.map?1565720446002